...
export async function GET(req: NextRequest) {
  ...
  const users = await prisma.user.findMany({
    select: { id: true, name: true, email: true, plan: true, active: true, createdAt: true },
...
export async function POST(req: NextRequest) {
  ...
  if (exists) {
    // Atualiza plano (e senha se fornecida)
    const data: any = { plan: plan || exists.plan }
    if (name) data.name = name
    if (password) data.password = await bcrypt.hash(password, 12)
    const updated = await prisma.user.update({ where: { id: exists.id }, data })
    return NextResponse.json({ ok: true, action: 'updated', user: { id: updated.id, email: updated.email, plan: updated.plan } })
  }
...
// PATCH /api/admin/users  → muda só o plano de um usuário existente
// Body: { key, email, plan }
