...
export default function AdminClient({ role, name }: { role: string; name: string }) {
  ...
  // Autenticação agora é por SESSÃO (login email+senha + papel admin/staff).
  // O cookie de sessão vai automático nas chamadas; sem chave/token.
    ...
    <div style={s.page}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ fontSize: 12, color: C.t3 }}>{name} · <span style={{ color: role === 'admin' ? C.gold : C.t2, textTransform: 'uppercase' as const, fontWeight: 700, fontSize: 10 }}>{role}</span> · {users.length} clientes</div>
