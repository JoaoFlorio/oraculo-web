...
const PLAN_CFG: Record<string,{label:string;color:string;glow:string;limit:number;tabs:string[];modal:boolean;export:boolean}> = {
  ...
  monthly:  { label:'Mensal',    color:T.pur, glow:'rgba(139,120,255,0.3)', limit:9999, tabs:['bestsellers','saved','competitor','ml-gestao','ml-minera','ml-calc','extension','agente','financeiro','tutoriais','perfil'], modal:true,  export:false },
  biannual: { label:'Semestral', color:T.gold,glow:'rgba(240,180,41,0.3)',  limit:9999, tabs:['bestsellers','saved','competitor','ml-gestao','ml-minera','ml-calc','extension','agente','financeiro','tutoriais','perfil'], modal:true,  export:true  },
  annual:   { label:'Anual',     color:T.g,   glow:'rgba(34,197,94,0.3)',   limit:9999, tabs:['bestsellers','saved','competitor','ml-gestao','ml-minera','ml-calc','extension','agente','financeiro','tutoriais','perfil'], modal:true,  export:true  },
  lifetime: { label:'Vitalício', color:T.g,   glow:'rgba(34,197,94,0.3)',   limit:9999, tabs:['bestsellers','saved','competitor','ml-gestao','ml-minera','ml-calc','extension','agente','financeiro','tutoriais','perfil'], modal:true,  export:true  },
}
...
const NAV_GROUPS = [
  ...
  { group:'Mercado Livre', ids:['ml-gestao','ml-minera','ml-calc'] },
...
function NavIcon({id,active}:{id:string,active:boolean}){
  ...
  const icons:Record<string,React.ReactElement> = {
    ...
    'ml-gestao': <><path d="M5 22V12M11 22V6M17 22V15M23 22V9" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/><path d="M5 8l6-3 6 5 6-4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></>,
...
export default function DashboardClient({user,gestaoEnabled=false}:{user:any;gestaoEnabled?:boolean}){
  ...
  // Quem pode abrir o painel /admin (o support vê só a tela de clientes lá). É
  // separado do isStaff de propósito: o support é cliente de verdade da loja
  // dele, então continua vendo aviso de vencimento do plano (não entra no isStaff).
  const canAdminPanel = isStaff || user.role === 'support'
  ...
  function goNav(id:string){
    ...
    if(id==='competitor'||id==='saved'||id==='perfil'||id==='ml-calc'||id==='ml-gestao'||id==='ml-minera'){loadIdRef.current++;setLoading(false);setProds([]);setDone(false);return}
    if(id==='extension'){
      ...
      <div className="ora-app" style={{display:'flex',height:'100vh',background:T.bg,color:T.t1,overflow:'hidden'}}>
        ...
        <aside className={`ora-side${mobileNav?' mopen':''}`} style={{width:sideOpen?248:64,background:T.sidebar,borderRight:`1px solid ${T.line}`,display:'flex',flexDirection:'column',transition:'width .22s cubic-bezier(.4,0,.2,1)',overflow:'hidden',flexShrink:0,zIndex:20,position:'relative' as const}}>
          ...
          <div style={{padding:'10px 12px 12px',borderTop:`1px solid ${T.line}`,flexShrink:0,display:'flex',flexDirection:'column',gap:8,position:'relative' as const}}>
            ...
            {/* Painel /admin — admin/staff (completo) ou support (só clientes) */}
            {canAdminPanel&&(()=>{ const lbl = user.role==='support' ? 'Clientes' : 'Painel Admin'; return (
              <a href="/admin" className="ora-wa-side" aria-label={lbl} title={!sideOpen?lbl:undefined}
                style={{display:'flex',alignItems:'center',justifyContent:sideOpen?'flex-start':'center',gap:9,padding:sideOpen?'9px 12px':'9px 0',borderRadius:10,textDecoration:'none',
                ...
                {sideOpen&&<span style={{fontSize:11,fontWeight:700,color:T.gold,letterSpacing:'0.02em',whiteSpace:'nowrap' as const}}>{lbl}</span>}
              </a>
            )})()}
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
          ...
          <main key={nav} ref={mainRef} className={`ora-tab-in${nav==='competitor'||nav==='agente'?'':' ora-main-pad'}`} style={{flex:1,overflowY:nav==='agente'?'hidden':'auto',padding:nav==='competitor'||nav==='agente'?'0':'28px 28px 40px',position:'relative' as const,display:'flex',flexDirection:'column'}}>
            ...
            {/* Gestão ML — DRE com os números REAIS cobrados pelo ML (tarifa do
                pedido + custo real do envio). Isolada da Gestão Amazon. */}
            {nav==='ml-gestao'&&(<MLGestao/>)}
            ...
            {nav==='agente'&&(
              <div style={{flex:1,minHeight:0,display:'flex',flexDirection:'column'}}>
                <NeoChat isAdmin={user.role==='admin'} userEmail={user.email}/>
              </div>
            )}
            ...
            {nav!=='competitor'&&nav!=='extension'&&nav!=='agente'&&nav!=='financeiro'&&nav!=='saved'&&nav!=='perfil'&&nav!=='tutoriais'&&nav!=='ml-calc'&&nav!=='ml-gestao'&&nav!=='ml-minera'&&<>
